...
>
      ...
      <div className="mx-auto flex min-h-[60vh] max-w-7xl items-center px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
        <div className="relative isolate mx-auto flex flex-col sm:flex-row items-center justify-center gap-6 sm:gap-10 text-center sm:text-left">
          ...
          <div className="relative block shrink-0 mr-2 sm:mr-6 lg:mr-10">
            <FlowerPortrait src={leftImg} alt="Cliente satisfecho disfrutando de comida" accentColor="#F0B100" />
          </div>
  ...
  <span className="text-gold drop-shadow-[0_0_8px_rgba(240,177,0,0.6)]">BENITO</span>{" "}
  <span>CONOCERÁS </span>
  <span className="text-gold drop-shadow-[0_0_8px_rgba(240,177,0,0.6)]">EL SABOR</span>{" "}
  <span>DE NUESTRA CARNE.</span>
          ...
          <div className="relative block shrink-0 ml-2 sm:ml-6 lg:ml-10">
            <FlowerPortrait src={rightImg} alt="Persona feliz en ambiente agradable" accentColor="#F0B100" />
          </div>
        </div>
      </div>
      ...
      <div className="relative">
        <div className="absolute inset-x-0 -bottom-10 z-10">
          <div className="w-full rotate-[-2deg] py-7 shadow-xl bg-gold">
            {/* Texto de la cinta: Negro en claro, Blanco en oscuro */}
       <div className="flex min-w-full shrink-0 items-center gap-8 sm:gap-12 whitespace-nowrap animate-[scrollX_60s_linear_infinite] text-lg sm:text-2xl font-extrabold uppercase tracking-[0.25em]"> 
              <ScrollingText />
            </div>
          </div>
        </div>
...
function FlowerPortrait({ src, alt, accentColor }) {
  return (
    <div className="relative h-24 w-24 sm:h-36 sm:w-36 md:h-40 md:w-40 animate-[floatY_6s_ease-in-out_infinite]">
      ...
      {/* Pétalos dorados */}
      {['-top-2 left-1/2 -translate-x-1/2', 'top-4 -left-2', 'top-4 -right-2', '-bottom-2 left-1/2 -translate-x-1/2'].map((pos, i) => (
        <span
          key={i}
...
function ScrollingText() {
  return (
    <>
      ...
      <div
        className="flex min-w-full shrink-0 items-center gap-12 whitespace-nowrap animate-[scrollX_60s_linear_infinite] text-2xl sm:text-3xl font-extrabold uppercase tracking-[0.25em]"
        aria-hidden="true"
      >
        {Array.from({ length: 16 }).map((_, i) => (
